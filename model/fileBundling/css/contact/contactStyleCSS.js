...
const contactCSS = () => {
  ...
  const body = fs.readFileSync('public/stylesheets/contact.css', {
    encoding: 'utf8',
    flag: 'r',
  });
  ...
  fs.writeFileSync('public/stylesheets/contact/style.css', variables, err => {
    if (err) throw 'an error occured writing header.css to home page style.css file'
  });
  fs.appendFileSync('public/stylesheets/contact/style.css', universalStyles, err => {
    if (err) throw 'an error occured writing home.css to home page style.css file'
  });
  fs.appendFileSync('public/stylesheets/contact/style.css', header, err => {
    if (err) throw 'an error occured writing footer.css home page style.css file'
  });   
  fs.appendFileSync('public/stylesheets/contact/style.css', body, err => {
    if (err) throw 'an error occured writing home.css to home page style.css file'
  });
  fs.appendFileSync('public/stylesheets/contact/style.css', footer, err => {
    if (err) throw 'an error occured writing footer.css home page style.css file'
  });
 }
