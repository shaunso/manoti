...
const equitiesCSS = () => {
  ...
  fs.writeFileSync('public/stylesheets/data/vfex/equities/style.css', variables, err => {
    if (err) throw 'an error occured writing header.css to home page style.css file'
  });
  fs.appendFileSync('public/stylesheets/data/vfex/equities/style.css', universalStyles, err => {
    if (err) throw 'an error occured writing home.css to home page style.css file'
  });
  fs.appendFileSync('public/stylesheets/data/vfex/equities/style.css', header, err => {
    if (err) throw 'an error occured writing footer.css home page style.css file'
  });  
  fs.appendFileSync('public/stylesheets/data/vfex/equities/style.css', body, err => {
    if (err) throw 'an error occured writing home.css to home page style.css file'
  });
  fs.appendFileSync('public/stylesheets/data/vfex/equities/style.css', footer, err => {
    if (err) throw 'an error occured writing footer.css home page style.css file'
  });
 }
